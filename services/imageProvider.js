...
const MODES = [
  ...
  'Create a fresh premium variation with refined detail, balanced density, sophisticated repetition, and production-friendly visual clarity.'
];

async function generateVariants({ imageBuffer, metadata, seedBase }) {
  if (PROVIDER === 'comfyui' || PROVIDER === 'h3') {
    return generateH3Variants({ imageBuffer, metadata, seedBase });
  }

  if (PROVIDER !== 'huggingface') {
    throw Object.assign(new Error('Unsupported provider'), {
      publicMessage: `Provider "${PROVIDER}" is not configured.`
    });
  }
  ...
  for (let i = 0; i < 4; i++) {
    ...
    try {
      ...
      else if (status === 402) publicMessage = 'Hugging Face requires available inference credits. Check your credits/billing.';
      else if (status === 404) publicMessage = `The selected model/provider is unavailable: ${MODEL} via ${HF_PROVIDER}.`;
